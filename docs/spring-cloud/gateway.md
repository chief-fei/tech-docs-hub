...
Spring Cloud Gateway 原生支持 WebSocket 协议的路由转发，无需额外配置即可透传 WebSocket 握手和帧数据。

### 9.1 WebSocket 路由原理

Gateway 基于 Netty 的异步非阻塞特性，天然支持长连接。WebSocket 请求到达 Gateway 后，Gateway 会：

1. 识别 `Upgrade: websocket` 请求头，判断是否为 WebSocket 握手
2. 匹配路由规则，将请求转发到后端 WebSocket 服务
3. 建立双向通道，后续数据帧在客户端 ↔ Gateway ↔ 后端服务之间透明传输

```text
客户端 ──WebSocket──▶ Gateway ──WebSocket──▶ 后端服务
   ◀────────── 双向数据帧透传 ──────────▶
```

### 9.2 基础配置

WebSocket 路由的配置与普通 HTTP 路由完全一致，只需将 `uri` 指向 WebSocket 服务即可：
...
spring:
  cloud:
    gateway:
      routes:
        - id: websocket-route
          uri: lb://ws-service          # 或 ws://localhost:8085
          predicates:
            - Path=/ws/**
```

::: tip WebSocket 路由不需要特殊断言
Gateway 会自动根据请求头中的 `Upgrade: websocket` 和 `Connection: Upgrade` 识别 WebSocket 请求，不需要额外的断言配置。
:::

### 9.3 完整配置示例

以 Spring WebSocket + SockJS 为例：
  ...
  cloud:
    gateway:
      routes:
        ...
        # SockJS 降级轮询路由（当 WebSocket 不可用时）
        - id: websocket-chat-sockjs
          uri: lb://chat-service
          predicates:
            - Path=/chat/**
...
::: warning SockJS 路径处理
如果后端使用 SockJS，客户端会访问 `/chat/123/xxx/websocket` 等路径。此时 `StripPrefix` 要谨慎使用——通常不需要去掉前缀，因为 SockJS 的路径结构与后端服务一致。
:::

### 9.4 跨域 WebSocket 配置

如果 WebSocket 客户端与 Gateway 不同域，需要配置 CORS：
...
spring:
  cloud:
    gateway:
      httpclient:
        websocket:
          # WebSocket 最大帧大小（默认 65536，即 64KB）
          max-frame-payload-length: 1048576   # 1MB
        # 连接超时和响应超时对 WebSocket 长连接同样适用
        connect-timeout: 5000
        response-timeout: 0                   # 0 表示永不超时（WebSocket 需要）
```

| 配置项 | 说明 | 推荐值 |
|--------|------|--------|
| `response-timeout` | 设为 `0` 禁用超时 | 0（WebSocket 长连接） |
| `max-frame-payload-length` | 最大帧大小 | 按业务需求设置 |
...
检查后端服务是否正常，确认 `response-timeout` 是否设置为 `0`（或足够大）。WebSocket 是长连接，默认的响应超时会断开连接。

**Gateway 后 WebSocket 无法获取真实客户端 IP？**

Gateway 转发 WebSocket 时会自动传递 `X-Forwarded-For` 头，后端可以通过该头获取真实 IP。
